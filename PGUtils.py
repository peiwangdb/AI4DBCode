...
def get_con(dbname, user, password, host, port):
...
class PGRunner:
    ...
    def getLatency(self, sql,sqlwithplan):
        ...
        """
        con, cur = get_con(self.dbname, self.user, self.password, self.host, self.port)
        ...
        cur.execute("SET statement_timeout = "+str(int(sql.timeout()))+ ";")
        ...
        if thisQueryCost / sql.getDPCost()<100:
            try:
                ...
                cur.execute("EXPLAIN ANALYZE "+sqlwithplan)
                rows = cur.fetchall()
    ...
    def getSelectivity(self,table,whereCondition):
        ...
        con, cur = get_con(self.dbname, self.user, self.password, self.host, self.port)
        cur.execute("SET statement_timeout = "+str(int(100000))+ ";")
